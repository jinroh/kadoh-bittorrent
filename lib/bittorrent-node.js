  ...
  handleGET_PEERS: function(rpc) {
    ...
    this._store.retrieve(rpc.getInfoHash())
        .then(function(values) {
          rpc.resolve(nodes, values.peers, 'deadbeef');
        }, function() {
  ...
  handleANNOUNCE_PEER: function(rpc) {
    ...
    this._store.retrieve(key)
        .pipe(function(values) {
          if (values.peers.every(function(peer) { return (peer[0] !== ip && peer[1] !== port); })) {
            values.peers.push([ip, port]);
            return this._store.save(key, values, -1);
          }
          return values;
        }, function() {
          return this._store.save(key, { peers: [[ip, port]] }, -1);
        }, this)
